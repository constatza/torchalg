...
class TestEuclideanNorm:
    ...
    def test_matches_hand_computed_value(self, three_four_vector: torch.Tensor) -> None:
        ...
        assert euclidean_norm(three_four_vector) == pytest.approx(5.0)

    def test_returns_python_float(self, three_four_vector: torch.Tensor) -> None:
        """Return type is a plain float, matching the ``Norm`` type alias."""
        assert isinstance(euclidean_norm(three_four_vector), float)

    def test_zero_vector_has_zero_norm(self, torch_dtype: torch.dtype) -> None:
        """The zero vector has norm 0."""
        assert euclidean_norm(torch.zeros(4, dtype=torch_dtype)) == 0.0
...
class TestEnergyNorm:
    ...
    def test_diagonal_case_matches_hand_computed_value(
        ...
        assert anorm(ones_vector_2) == pytest.approx(math.sqrt(10.0))
    ...
    def test_diagonal_and_full_branches_agree(
        ...
        assert diag_norm == pytest.approx(full_norm)
